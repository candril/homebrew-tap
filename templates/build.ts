...
if (olderThan(Bun.version, BUN_FLOOR)) {
  console.error(`{{tool}} needs bun ${BUN_FLOOR} or newer to build; this is ${Bun.version}`)
  process.exit(1)
}
...
// `dist/{{tool}}`; CI and `--all` builds carry the platform in the name. (Copying the
...
for (const target of targets) {
  const outfile = localBuild ? "dist/{{tool}}" : `dist/{{tool}}-${target.os}-${target.arch}`
  console.log(`{{tool}} ${version} → ${outfile}`)
  const result = await Bun.build({
    entrypoints: ["{{entry}}", workerPath],
    target: "bun",
    compile: { target: `bun-${target.os}-${target.arch}`, outfile },
    define: {
      OTUI_TREE_SITTER_WORKER_PATH: JSON.stringify(workerInBinary),
      {{TOOL}}_VERSION: JSON.stringify(version),
    },
  })
  if (!result.success) {
