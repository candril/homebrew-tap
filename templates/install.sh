...
# {{tool}} installer
# Usage: curl -fsSL https://raw.githubusercontent.com/candril/{{tool}}/main/scripts/install.sh | bash
#
#   {{TOOL}}_VERSION=0.2.0              install a specific release instead of the latest
#   {{TOOL}}_INSTALL_DIR=~/.local/bin   install somewhere other than /usr/local/bin
#
# Homebrew and Nix users have shorter routes:
#   brew install candril/tap/{{tool}}
#   nix run github:candril/{{tool}}
#
# This file is generated from candril/homebrew-tap/templates/install.sh; edit it there.

REPO="candril/{{tool}}"
INSTALL_DIR="${{{TOOL}}_INSTALL_DIR:-/usr/local/bin}"
BINARY_NAME="{{tool}}"
...
if [ -n "${{{TOOL}}_VERSION:-}" ]; then
  TAG="v${{{TOOL}}_VERSION#v}"
  echo "Installing ${TAG}..."
else
...
if [ -w "$INSTALL_DIR" ]; then
  mv "{{tool}}-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
else
  echo "  ${INSTALL_DIR} is not writable — using sudo"
  sudo mv "{{tool}}-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
fi

echo
echo "{{tool}} ${TAG} installed to ${INSTALL_DIR}/${BINARY_NAME}"
if ! command -v {{tool}} >/dev/null 2>&1; then
  echo "Note: ${INSTALL_DIR} is not on your PATH."
fi
echo "Run '{{tool}} --help' to get started."
