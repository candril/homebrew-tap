...
# There is deliberately no flake.lock here: `nix run github:candril/{{tool}}` should
...
{
  description = "{{tool}} — packaged from its GitHub releases via candril/homebrew-tap";
  ...
  outputs = { self, tap, nixpkgs }:
    ...
    in {
      packages = forAll (system: {
        default = tap.packages.${system}.{{tool}};
      });
